...
resource "aws_kms_key" "s3_kms_key" {
  count  = (var.static_website_hosting || !var.create_kms_key) ? 0 : 1   # create key only if NOT creating a static website
...
resource "aws_s3_bucket_policy" "https_policy" {
  count  = (var.static_website_hosting || !var.enforce_encryption_in_transit) ? 0 : 1   # Apply policy only if not creating a static website  
